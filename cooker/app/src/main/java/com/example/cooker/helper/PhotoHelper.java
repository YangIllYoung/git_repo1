package com.example.cooker.helper;
...
public class PhotoHelper {
    ...
    public String getNewPhotoPath() {
        ...
        if (!dir.exists()) {    // 폴더가 없으면 만들기
            dir.mkdirs();   // 경로가 없어도 디렉토리 만드는 함수
        }
        Log.d("[INFO]", "dir = " + dir);
   ...
   public Bitmap getThumb(Activity activity, String path){
       ...
       /* 3. 이미지 리사이징 */
       // 이미지의 긴축의 길이가 스마트폰의 긴축 길이보다 크면
       if (fscale > maxScale) {
           // 이미지의 사이즈를 masScale로 나누어서 샘플링사이즈 계산
           // ex) 이미지의 긴축이 2400px, 스마트폰의 긴축이 800px 일 때,
           //       2400 / 800 = 3          => 샘플링 사이즈 = 3
           int sampleSize = fscale/maxScale;
           // 사진을 불러오기 직전에, 새 비트맵 옵션 생성
           BitmapFactory.Options options2 = new BitmapFactory.Options();
           // 샘플 사이즈 설정 - > 3으로 지정하면, 1/3 크기가 된다.
           options2.inSampleSize = sampleSize;
           // 이미지 읽어오기 - sampleSize만큼 크기를 줄여서 읽어온다.
           bmp = BitmapFactory.decodeFile(path, options2);  // ->options2를 지정해주지 않으면 원래 사이즈로 들고온다.
       } else {
